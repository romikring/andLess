...
public class AndLessSrv extends Service {
    ...
    /*
     * public native int audioInit(int ctx, int mode); public native boolean	audioExit(int ctx); public native boolean
     * audioStop(int ctx); public native boolean	audioPause(int ctx); public native boolean	audioResume(int ctx);
     *
     * public native int	audioGetDuration(int ctx); public native int	audioGetCurPosition(int ctx); public native boolean
     * audioSetVolume(int ctx, int vol);
     *
     * public native int	alacPlay(int ctx,String file, int start); public native int	flacPlay(int ctx,String file, int start);
     * public native int	apePlay(int ctx,String file, int start); public native int	wavPlay(int ctx,String file, int start);
     * public native int	wvPlay(int ctx,String file, int start); public native int	mpcPlay(int ctx,String file, int start); public
     * native int []	extractFlacCUE(String file);
     */
        ...
        for (int i = 0; i < k; i++) {
            try {
                if (!error) {
                    ...
                    AndLessSrv.this.notify(R.drawable.play_on/*
    ...
    private static final IAndLessSrv.Stub binder = new IAndLessSrv.Stub() {
        ...
        public void registerCallback(IAndLessSrvCallback cb) {
            if (cb != null) {
                cBacks.register(cb);
            }
        }
        ...
        ;
		public void unregisterCallback(IAndLessSrvCallback cb) {
            if (cb != null) {
	...
	private class Launcher {
        ...
        void launch(String path) {
            startActivity((new Intent()).setAction(AndLessSrv.ACTION_VIEW).setData(Uri.fromFile(new File(path))));
        }
    }
